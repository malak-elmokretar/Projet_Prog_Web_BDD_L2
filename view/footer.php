    <footer class=" text-white text-center mt-auto py-3">
      <p class="mb-0">
    <i class="bi bi-c-circle-fill"></i>
    2026, retrouvez nous sur :
    <a href="https://www.instagram.com" class="text-white ms-2" target="_blank" aria-label="Instagram">
        <i class="bi bi-instagram"></i>
    </a>
    <a href="#" onclick="document.getElementById('bandeau-cookies').style.display='block'; return false;" 
        class="text-white ms-2" aria-label="Gérer les cookies">
        <i class="bi bi-cookie"></i>
    </a>
    <a href="<?php echo $racine.'control/mentionslegales.php'; ?>" class="text-white ms-2">
        Mentions légales
    </a>
</p>
    <?php if (cookieAccepte('cookie_statistiques')): ?>  
        <p class="mb-0 mt-1" style="font-size:0.85em;">
            <i class="bi bi-eye"></i>
            Vous avez visité ce site <?php echo $_COOKIE['nb_visites'] ?? 0; ?> fois.
        </p>
    <?php endif; ?>
    </footer>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js" integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI" crossorigin="anonymous"></script>
  </body>
</html>